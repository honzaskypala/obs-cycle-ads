...
class AdCycler:
    ...
    def ticker(self):
        if self.timer_on:
            ...
            ads = tuple(filter(lambda x: x[:len(self.prefix)] == self.prefix, [S.obs_source_get_name(i) for i in S.obs_enum_sources()]))
            if len(ads) == 1:  # if only one ad, then just restart playing the video
                ...
                for ad in ads:
                    if visible_ad_found:
                        ad_to_show = ad
                        break
                    scene_item = S.obs_scene_find_source_recursive(current_scene, ad)
                    if S.obs_sceneitem_visible(scene_item):
                        S.obs_sceneitem_set_visible(scene_item, False)
                        visible_ad_found = True
                S.obs_sceneitem_set_visible(S.obs_scene_find_source_recursive(current_scene, ad_to_show), True)
...
def script_defaults(settings):
    ...
    S.obs_data_set_default_string(settings, "prefix", DEFAULT_PREFIX)

def script_update(settings):
    ad_cycler.interval = S.obs_data_get_int(settings, "interval")
    ad_cycler.prefix = S.obs_data_get_string(settings, "prefix")
...
def script_properties():  # ui
    ...
    prefix = S.obs_properties_add_text(props, "prefix", "Source prefix:", S.OBS_TEXT_DEFAULT)
    return props
